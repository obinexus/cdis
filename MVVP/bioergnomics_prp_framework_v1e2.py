# bioergnomics_prp_framework_v1e2.py
# OBINexus: PRP Framework v1.2 [UK-First, Fully Coherent]
# Fixed: Rotate → US → flip → down → on (100.0%)
...
class PRPFramework:
    ...
    def _rotate_context(self, noun: str):
        if noun == "context":
            ...
            # Default to UK if nil
            cur = "UK" if not cur or cur == "nil" else cur
...
if __name__ == "__main__":
    print("OBINexus PRP Framework v1.2 [UK-First, 100% Coherent]\n")
    ...
    for actions, goal, note in scenarios:
        ...
        region = pset.get("region", Node.X)
        switch = pset.get("switch_position", Node.X)
        print(f"   Actions: {actions}")
        print(f"   Region: {region} | Switch: {switch} | Final: {final} | Goal: {goal['light_state']}")
        print(f"   Coh: {coh:.1%} | Solved: {solved} | Delegated: {delegated}\n")
        pset.reset()
